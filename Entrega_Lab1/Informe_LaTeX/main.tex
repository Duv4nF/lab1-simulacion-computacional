...
\newcommand{\codigoEstudiante}{[código estudiantil]}
...
Esta práctica persigue cuatro objetivos concretos, tomados de la guía de laboratorio:

\begin{itemize}[itemsep=2pt]
    \item Introducir la simulación computacional mediante un caso \emph{ad hoc}: un sistema de cola simple con un solo servidor y un límite máximo en el número de clientes atendidos.
    \item Generar las variables aleatorias del modelo ---tiempo entre llegadas y tiempo de servicio--- a partir de mecanismos de sorteo equivalentes a dados y ruletas, con distribución uniforme discreta.
    \item Calcular las cinco medidas de desempeño definidas en el Capítulo 1 de \cite{Banks1998} e interpretar lo que cada una dice sobre el sistema.
    \item Repetir la simulación diez veces en dos escenarios de distinta longitud (20 y 200 clientes) para construir intervalos de confianza al 95\,\% y al 99\,\%, y analizar cómo cambia la precisión de las estimaciones.
\end{itemize}
...
Es una función que asigna un número real a cada resultado posible del espacio muestral de un experimento aleatorio. Dicho de otro modo, traduce resultados ---que pueden ser cualitativos--- a valores numéricos con los que sí se puede operar. Se llama \emph{discreta} cuando su recorrido es finito o infinito numerable, y \emph{continua} cuando puede tomar cualquier valor dentro de un intervalo de los reales.

En esta práctica intervienen dos variables aleatorias discretas: el tiempo que transcurre entre la llegada de un cliente y la del siguiente, y el tiempo que el cajero tarda en atender a un cliente. Ninguna de las dos se conoce de antemano; de ahí que la simulación necesite \emph{sortearlas} en cada repetición en lugar de fijarles un valor.
...
El divisor $n-1$ ---y no $n$--- es lo que distingue la desviación estándar \emph{muestral} de la poblacional: corrige el sesgo que aparece al estimar la dispersión usando una media que también fue estimada de la misma muestra. En el cuaderno de Python esto corresponde al argumento \texttt{ddof=1}.
...
Conviene notar la diferencia entre las dos últimas medidas de espera: la tercera reparte el tiempo total de cola entre \emph{todos} los clientes, incluidos los que no esperaron nada, mientras que la quinta lo reparte solo entre quienes efectivamente hicieron fila. Por construcción la quinta es siempre mayor o igual que la tercera, y el cociente entre ambas es exactamente la fracción que esperó.
...
Ahora bien, sustituir el dado por un algoritmo obliga a demostrar que el sustituto se comporta como el original. Para eso se aplicó una prueba $\chi^2$ de bondad de ajuste sobre la totalidad de los valores generados en cada escenario, contrastando las frecuencias observadas de cada resultado contra las esperadas bajo uniformidad. Los resultados aparecen en la Tabla~\ref{tab:valgen}.
...
La Tabla~\ref{tab:sim20} presenta completa la primera de las diez corridas. Las columnas (2) y (4) contienen los valores sorteados; las siete restantes se obtuvieron aplicando fila por fila las recurrencias de la sección anterior. El primer cliente no tiene tiempo entre llegadas ---llega en el instante cero, que es el origen del reloj de simulación--- y por eso esa celda aparece vacía.

\input{tbl20.tex}

Vale la pena seguir un par de filas para ver el mecanismo en acción. El cliente 7 llega en el minuto 38, cuando el cajero todavía está atendiendo al cliente 6 hasta el minuto 39; su atención empieza entonces un minuto más tarde que su llegada, y ese minuto aparece en la columna (9). El caso opuesto ocurre entre los clientes 4 y 5: el servidor queda libre en el minuto 21 y el siguiente cliente no aparece hasta el 28, de modo que la columna (8) registra 7 minutos de inactividad. El cliente 16 concentra el peor momento de la corrida: llega en el minuto 77, espera 4 minutos porque hay dos personas delante y su servicio dura 6, con lo que permanece 10 minutos en el sistema, el máximo de la tabla.
...
La lectura conjunta es la interesante: el sistema está holgado ---el servidor está libre buena parte del tiempo--- y sin embargo, cuando la congestión aparece, es perceptible para el cliente que la sufre. Esa aparente contradicción es característica de los sistemas de espera y se explica porque las llegadas no están repartidas de manera uniforme en el tiempo, sino que se agrupan por azar.
...
De ahí la conclusión metodológica: reportar una sola corrida como si fuera \guillemotleft el resultado\guillemotright{} de la simulación sería engañoso. Habría bastado con presentar la corrida 4 o la 7 para dar dos retratos casi opuestos del mismo banco. Hace falta repetir el experimento y cuantificar la incertidumbre, que es justamente lo que hace la sección siguiente.
...
Comparadas con su respectiva media, las $S$ obtenidas son grandes. El caso más severo es la espera promedio por cliente, con $S = 0{,}8430$ frente a $\bar{X} = 1{,}1750$: la dispersión equivale al 72\,\% de la media. Le siguen la fracción que esperó (41\,\%) y la espera de quienes esperaron (33\,\%). Las medidas más estables son el tiempo en el sistema y el porcentaje ocioso, ambas alrededor del 25\,\%. Esto tiene una explicación: las medidas que se calculan sobre los 20 clientes de la corrida promedian más observaciones que las que se calculan solo sobre los pocos clientes que esperaron ---en la corrida 1 fueron apenas 6---, y menos observaciones significa más ruido.
...
La corrida se extendió por 1088 minutos ---algo más de 18 horas de operación--- durante los cuales se acumularon 941 minutos de permanencia en el sistema, 391 de inactividad del servidor y 244 de espera en cola. Con diez veces más clientes, también aparecen situaciones que la corrida corta no alcanzó a mostrar: la cola más larga registrada hizo esperar 9 minutos a un cliente, frente a un máximo de 4 minutos en el escenario de 20 clientes.
...
La diferencia con la Tabla~\ref{tab:runs20} es contundente. El tiempo promedio en el sistema ahora se mueve entre 4,11 y 5,05 minutos ---un rango de 0,95 minutos--- frente a los 3,6 minutos de rango que se observaron con 20 clientes: la variación entre corridas se redujo a poco más de la cuarta parte. El porcentaje de tiempo ocioso se concentra entre 32,65\,\% y 41,13\,\%, siempre rondando el 36\,\% teórico, mientras que antes llegaba a bajar al 20,95\,\%. Ya no hay corridas atípicas como la número 7 del escenario A.

La similitud, en cambio, está en las medias: 4,60 minutos en el sistema aquí frente a 4,66 allá; 3,11 minutos de espera para quienes esperaron en ambos escenarios, con una diferencia de apenas una milésima. Y esto es exactamente lo que debía ocurrir. Alargar la corrida \textbf{no cambia el sistema} ---las distribuciones de llegada y de servicio son las mismas--- de modo que el valor esperado de cada medida no tiene por qué moverse. Lo que cambia es la precisión con que ese valor se estima. Dicho de otro modo: los dos escenarios apuntan al mismo blanco, pero el de 200 clientes tiene mejor puntería.
...
Hay un detalle que vale la pena señalar. Si las medidas de cada corrida fueran promedios de observaciones independientes, multiplicar por diez la longitud de la corrida debería dividir la desviación estándar por $\sqrt{10} = 3{,}162$. Las razones $S_{20}/S_{200}$ efectivamente obtenidas son 4,27 (tiempo en el sistema), 3,23 (tiempo ocioso), 3,47 (espera por cliente), 2,66 (fracción que esperó) y 3,49 (espera de quienes esperaron): todas se agrupan alrededor de ese 3,16 teórico. La coincidencia no es exacta ---y no tenía por qué serlo, porque los tiempos de espera de clientes consecutivos están correlacionados entre sí y porque diez corridas son pocas para estimar bien una desviación estándar--- pero confirma que el mecanismo que reduce la incertidumbre es el esperado.
...
El paso del 95\,\% al 99\,\% conserva el mismo factor de 1,437 del escenario anterior ---no podía ser de otro modo, porque $n$ es igual en ambos---, pero ahora ese ensanchamiento se aplica sobre intervalos mucho más angostos y sale mucho más barato: el del tiempo en el sistema crece de 0,38 a 0,54 minutos y sigue siendo estrecho. Ahí está la ventaja práctica de trabajar con corridas largas: permite exigir un nivel de confianza alto sin sacrificar precisión útil.
...
Como las cinco medidas están expresadas en unidades distintas ---minutos unas, proporciones otras--- comparar sus $S$ entre sí sería tramposo: favorecería automáticamente a las que se miden en una escala más pequeña. La Tabla~\ref{tab:cv} corrige eso presentando la dispersión y la precisión en términos relativos, que son adimensionales y sí admiten comparación cruzada.
...
En síntesis, la combinación óptima es \textbf{corridas largas con el menor nivel de confianza que resulte aceptable} para la decisión que se quiera tomar. Y aquí conviene una advertencia: el intervalo más estrecho no es automáticamente el mejor. Un intervalo del 95\,\% es más angosto que uno del 99\,\% simplemente porque acepta equivocarse una de cada veinte veces en lugar de una de cada cien. La estrechez solo es una virtud cuando proviene de reducir la variabilidad ---como al pasar de 20 a 200 clientes---, no cuando proviene de rebajar la exigencia de confianza.
...
\begin{enumerate}[itemsep=4pt]
    ...
    \item Alargar la corrida de 20 a 200 clientes no modificó las medias estimadas ---4,66 frente a 4,60 minutos en el sistema--- pero redujo las desviaciones estándar muestrales entre un 62\,\% y un 77\,\%. Las razones de reducción observadas se agruparon en torno al valor $\sqrt{10} = 3{,}162$ que predice la teoría, lo que confirma que la ganancia de precisión proviene de promediar más observaciones y no de un cambio en el sistema simulado.

    \item Los intervalos de confianza cuantifican esa ganancia de manera directa. El margen relativo del tiempo promedio en el sistema pasó de $\pm 17{,}3\,\%$ con 20 clientes a $\pm 4{,}1\,\%$ con 200, y el de la espera promedio por cliente ---la medida más ruidosa--- de $\pm 51{,}3\,\%$ a $\pm 15{,}6\,\%$.
